...
And that deep understanding didn't just stay in theory. I built [AnonimSor](https://anonimsor.com), a real-time anonymous Q&A platform (currently in pre-launch). I built [FIGHTLAND](https://fightland.vercel.app), a custom Rust game server with a full economy. I built [Ficksa](https://ficksa.com), an e-commerce platform (currently in beta).
...
And beyond HSRCPay, I've built other products. E-commerce systems (Ficksa, in beta). Social platforms (AnonimSor, pre-launch). Gaming infrastructure (FIGHTLAND). Some succeeded. Some are still being built. Each project taught me something. Each one validated different parts of my approach.
